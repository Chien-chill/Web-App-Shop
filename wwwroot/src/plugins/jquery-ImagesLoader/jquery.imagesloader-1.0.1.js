    ...
    $(document).ready(function () {
      ...
      var imagesloader = $('[data-type=imagesloader]').imagesloader({
        ...
        $("#submit").click(function (event) {
            ...
            // Tạo DataTransfer để chứa danh sách file
            var dataTransfer = new DataTransfer();

            self.AttachmentArray.forEach(fileObj => {
                if (fileObj.File) {
                    dataTransfer.items.add(fileObj.File);
                }
            });

            // Gán danh sách file vào input
            self.$files[0].files = dataTransfer.files;
            console.log("self.$files sau khi cập nhật:", self.$files[0].files);
        });
